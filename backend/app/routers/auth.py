...
def login(body: LoginRequest, sb: SupabaseClient = Depends(get_supabase)):
    ...
    if not result.data or not verify_password(body.password, result.data["senha_hash"]):
...
def register(body: RegisterRequest, sb: SupabaseClient = Depends(get_supabase)):
    ...
    sb.table("usuarios").insert({"email": body.email, "senha_hash": hash_password(body.password)}).execute()
